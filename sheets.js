...
async function updateSheet(auth, spreadsheetId) {
  ...
  const current = await orders.find(
      ...
      "orderID client manager status date itemTitle amount platform refundStatus refundData key"
    ),
    updates = current.sort((one, two) =>
      Number(one.date) > Number(two.date) ? -1 : 1
    );
