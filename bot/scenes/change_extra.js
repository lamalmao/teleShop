...
changeExtra.enterHandler = async function(ctx) {
  try {
    ...
    await ctx.telegram.editMessageCaption(
      ...
      msg,
      {
        reply_markup: Markup.inlineKeyboard([
          ...
          [ Markup.button.callback('Изменить варианты ответа', 'change_answers') ],
    ...
    ctx.scene.enter('showGoods', { menu: ctx.scene.state.menu, category: ctx.scene.state.category, item: ctx.scene.state.item });
  }
};
...
changeExtra.action(keys.BackMenu.buttons, async ctx => {
  try {
    ...
    } else ctx.scene.enter('manageItem', { menu: ctx.scene.state.menu, category: ctx.scene.state.category, item: ctx.scene.state.item });
