...
const userRefund = new Scenes.WizardScene(
  "user_refund",
  async (ctx) => {
    try {
      const orderID = ctx.scene.state.refundID
        ? ctx.scene.state.refundID
      ...
      null;
      ctx
        .answerCbQuery("Что-то пошло не так, попробуйте снова")
        .catch((_) => null);
      ctx.scene.leave();
    }
  },
  async (ctx) => {
    try {
      if (ctx.updateType === "callback_query") {
        if (
          ctx.callbackQuery.data === "qiwi" ||
          ctx.callbackQuery.data === "card"
        ) {
          ctx.scene.state.target = ctx.callbackQuery.data;
          msg =
            messages.refund_instructions[
              ctx.callbackQuery.data === "qiwi" ? 1 : 2
            ];
          ...
          ctx
            .answerCbQuery("Не та кнопка, выберите платежную систему")
            .catch((_) => null);
      }
    } catch (e) {
      null;
    ...
    try {
      if (ctx.updateType === "message") {
        const value = ctx.message.text.replace(/[\+\- \(\)]/g, ""),
          wallet =
            ctx.scene.state.target === "qiwi" ? value : value.substring(0, 16);

        null;

        if (
          (ctx.scene.state.target === "card" && /\d{16}/.test(wallet)) ||
          (ctx.scene.state.target === "qiwi" && /\d+/.test(wallet))
        ) {
          ...
          await ctx.telegram.editMessageText(
            ...
            undefined,
            `<b>Все верно?</b>\n\n<b>${
              ctx.scene.state.target === "qiwi" ? "QIWI" : "Банковская карта"
            }</b> ${
              ctx.scene.state.wallet
  ...
  async (ctx) => {
    try {
      if (
        ctx.updateType === "callback_query" &&
        ctx.callbackQuery.data === "refund_success"
      ) {
