...
editCategory.on('callback_query',
  async (ctx, next) => {
    ...
    try {
      if (ctx.scene.state.method === 'callback') {
        if (ctx.scene.state.target === 'delete') {
          ...
          } else ctx.scene.reenter('editCategory', { category: ctx.scene.state.category, message: ctx.scene.state.message, keyboard: ctx.scene.state.keyboard });
          ...
          if (ctx.callbackQuery.data === keys.YesNoMenu.buttons.yes) {
            console.log(ctx.scene.state.category.hidden, !ctx.scene.state.category.hidden);
            ...
            ctx.answerCbQuery('Видимость категории изменена').catch(_ => null);
          }

          ctx.scene.reenter({ category: ctx.scene.state.category, message: ctx.scene.state.category, keyboard: ctx.scene.state.keyboard });
        } else {
          ctx.scene.state.category[ctx.scene.state.target] = ctx.callbackQuery.data;
          await categories.updateOne({
            _id: ctx.scene.state.category._id
          }, ctx.scene.state.category);

          ctx.answerCbQuery('Новое значение установлено').catch(_ => null);
          ctx.telegram.deleteMessage(ctx.from.id, ctx.scene.state.menu.message_id);
      ...
      console.log(e);
      ctx.deleteMessage(ctx.callbackQuery.message.message_id).catch(_ => null);
    ...
    try {
      ...
      switch (ctx.callbackQuery.data) {
        ...
        case 'relocate':
          ...
          var keyboard = [];
          const mainCategories = await categories.find({ type: 'main' }, '_id title');
        ...
        case 'rerender':
          ...
          ctx.telegram.editMessageCaption(ctx.from.id, ctx.scene.state.menu.message_id, undefined, 'Подождите...');
          const newBlankFileName = await render.renderShopPage(ctx.scene.state.category._id);
          const newCategoryImageFileName = await render.genImageFromHTML(newBlankFileName);
      ...
      if (!nowshow) {
        ...
        if (method === 'callback') await ctx.telegram.editMessageReplyMarkup(ctx.from.id, ctx.scene.state.menu.message_id, undefined, keyboard.reply_markup);
      }
    } catch (e) {
      console.log(e);
      ctx.deleteMessage(ctx.callbackQuery.message.message_id).catch(_ => null);
  ...
  async ctx => {
    try {
      ...
      axios({
        ...
        writer.on('error', err => {
          problem = true;
          ctx.telegram.answerCbQuery(ctx.scene.state.cb, `Ошибка: ${err.message}`).catch(_ => null);
      ...
      console.log(e);
      ctx.deleteMessage(ctx.scene.state.menu.message_id).catch(_ => null);
      ctx.telegram.answerCbQuery(ctx.scene.state.cb, 'Что-то пошло не так').catch(_ => null);
      ctx.scene.reenter({ category: ctx.scene.state.category, message: ctx.scene.state.category, keyboard: ctx.scene.state.keyboard });
    }
  }
);

editCategory.on('message',
  async (ctx, next) => {
    ctx.deleteMessage().catch(_ => null);
    var msg;
    if (ctx.message.text) msg = ctx.message.text.trim();

    if (!msg) ctx.telegram.answerCbQuery(ctx.scene.state.cb, 'Пустое сообщение').catch(_ => null);
  ...
  async ctx => {
    try {
      ...
      ctx.telegram.answerCbQuery(`Новое значение установлено`).catch(_ => null);
