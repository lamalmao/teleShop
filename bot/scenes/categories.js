...
categories.enterHandler = async function (ctx) {
  try {
    ...
    await ctx.telegram.editMessageReplyMarkup(
      ...
      message,
      undefined,
      keys.CategoriesManageMenu.keyboard.reply_markup
    );
  } catch (e) {
    null;
...
categories.on("callback_query", async (ctx) => {
  try {
    ...
    if (cb === keys.CategoriesManageMenu.buttons.back)
      ctx.scene.enter("admin", ctx.scene.state);
    else
      ctx.scene
        .enter(cb, ctx.scene.state)
        .catch((_) =>
          ctx.answerCbQuery("Неизвестная ошибка").catch((_) => null)
        );
  } catch (e) {
    null;
