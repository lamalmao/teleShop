  ...
  try {
    ...
    for (let payment of story.sort((a, b) => a.date > b.date ? 1 : -1)) {
      ...
      if (payment.itemTitle) {
        keyboard.push(([ Markup.button.callback(`-${payment.amount}₽ - ${payment.itemTitle}`, `user_order#${payment.orderID}`) ]));
      } else {
        keyboard.push([ Markup.button.callback(`+${payment.amount}₽ от ${date}`, `user_payment#${payment.paymentID}`) ]);
      }
    }
