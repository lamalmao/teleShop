...
showCategories.on(
  "callback_query",
  async (ctx, next) => {
    if (ctx.callbackQuery.data === keys.BackMenu.buttons)
      ctx.scene.enter("categories", { menu: ctx.scene.state.menu });
    else next();
  },
  async (ctx) => {
    try {
      const chosen = ctx.scene.state.categories.find(
        (c) => c._id == ctx.callbackQuery.data
      );
...
async function genCategoryData(category) {
  ...
  if (category.type === "main")
    keyboard.push([cb("Изменить обложку", "editBackground")]);
