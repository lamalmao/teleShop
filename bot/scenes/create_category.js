...
const typeChoice = Markup.inlineKeyboard([
  [
    Markup.button.callback("Основная", "main"),
    Markup.button.callback("Вложенная", "sub"),
  ],
  [Markup.button.callback("Назад", keys.BackMenu.buttons)],
]);

const createCategory = new Scenes.WizardScene(
  "createCategory",
  async (ctx) => {
    try {
      ...
      if (ctx.updateType === "message") {
        ...
        if (description === "") {
          ...
          await ctx.telegram.editMessageText(
            ...
            message,
            undefined,
            "Укажите тип категории"
          );
          await ctx.telegram.editMessageReplyMarkup(
            ctx.from.id,
            message,
            undefined,
            typeChoice.reply_markup
          );
  ...
  async (ctx) => {
    try {
      ...
      if (cb) {
        if (cb.data === keys.BackMenu.buttons)
          ctx.scene.enter("categories", ctx.scene.state);
        else {
          ...
          if (cb.data === "sub") {
            ...
            if (cores.length === 0) {
              ...
              for (let category of cores) {
                mainCategoriesListKeyboard.push([
              ...
              await ctx.telegram.editMessageText(
                ...
                message,
                undefined,
                "Выберите куда вложить новую подкатегорию"
              );
              await ctx.telegram.editMessageReplyMarkup(
                ctx.from.id,
                message,
                undefined,
                Markup.inlineKeyboard(mainCategoriesListKeyboard).reply_markup
              );
            ...
            await ctx.telegram.editMessageText(
              ...
              message,
              undefined,
              "Загрузите обложку категории (в случае пропуска будет использоваться изображение с главного меню)"
            );
            await ctx.telegram.editMessageReplyMarkup(
              ctx.from.id,
              message,
  ...
  async (ctx) => {
    try {
      if (ctx.updateType === "callback_query") {
        ...
        if (cb === keys.BackMenu.buttons)
          ...
          ctx.telegram
            .answerCbQuery(
              ctx.scene.state.cb,
              `Вложенная категория ${ctx.scene.state.newCategory.title} успешно создана`
            )
            .catch((_) => null);
          ctx.scene.enter("categories", { menu: ctx.scene.state.menu });
        }
      ...
      ctx.telegram
        .answerCbQuery(
          ctx.scene.state.cb,
          `Ошибка во время создания подкатегории: ${e.message}`
        )
        .catch((_) => null);
      ctx.scene.enter("categories", { menu: ctx.scene.state.menu });
    }
  ...
  async (ctx) => {
    try {
          ...
          axios({
            ...
            .then((res) => {
              ...
              writer.on("error", async (err) => {
                ...
                ctx
                  .answerCbQuery(
                    `Ошибка во время загрузки файла: ${err.message}`
                  )
                  .catch((_) => null);
              ...
              writer.on("close", async () => {
                if (!problem) {
                  ...
                  ctx.telegram
                    .answerCbQuery(
                      ctx.scene.state.cb,
                      `Категория ${ctx.scene.state.newCategory.title} успешно создана`
                    )
                    .catch((_) => null);
                  ctx.scene.enter("categories", { menu: ctx.scene.state.menu });
                }
            ...
            .catch(async (err) => {
              ...
              ctx.telegram
                .answerCbQuery(
                  ctx.scene.state.cb,
                  "Что-то пошло не так при загрузке изображения"
                )
                .catch((_) => null);
              ctx.deleteMessage().catch((_) => null);
              ctx.scene.enter("categories", { menu: ctx.scene.state.menu });
      ...
      ctx.telegram
        .answerCbQuery(
          ctx.scene.state.cb,
          `Ошибка во время создания категории: ${e.message}`
        )
        .catch((_) => null);
      ctx.scene.enter("categories", { menu: ctx.scene.state.menu });
    }
