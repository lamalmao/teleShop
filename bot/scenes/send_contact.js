...
sendContact.enterHandler = async function (ctx) {
  try {
    ...
    await ctx.reply(
      "Отравьте нам пожалуйста свой контакт (при помощи кнопки ниже), чтобы мы могли с вами связаться по поводу заказа\n\nЕсли после отправки вам никто не напишет, пожалуйста обратитесь сюда: @fbzmanager",
      {
...
sendContact.on("contact", async (ctx) => {
  try {
    ...
    ctx.telegram
      .sendContact(global.managerID, ctx.message.contact.phone_number, msg)
      .catch((_) => null);
